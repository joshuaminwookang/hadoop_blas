/**
 * MapReduce job(s) to calculate click through rates for each (referrer url, ad_id)
...
public class BetterMM {
	...
	job1.setMapperClass(BetterMM.FirstMapper.class);
	job1.setReducerClass(BetterMM.FirstReducer.class);
	...
	job2.setJarByClass(BetterMM.class);
	job2.setJobName("step 2");

	// set the Mapper and Reducer functions we want
	job2.setMapperClass(BetterMM.SecondMapper.class);
	job2.setReducerClass(BetterMM.SecondReducer.class);
	...
	public void map(LongWritable key, Text val, Context context)
	    ...
	    if (filename.equals("ma.csv")){
		intermediateKey = index.toString();
		for (int i = 0; i < n; i++) {
		    intermediateKey += "A\t"+index.toString()+"\t"+i.toString()+"\t"+vals[i]+"\n";
		}
	    } else {
		intermediateKey= (n+index).toString();
		for (int i = 0; i < n; i++) {
		    intermediateKey += "B\t"+index.toString()+"\t"+i.toString()+"\t"+vals[i]+"\n"
		}
	    }
	    // Write to disk
